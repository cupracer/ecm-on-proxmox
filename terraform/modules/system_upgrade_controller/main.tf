resource "ssh_resource" "setup_system_upgrade_controller" {
  ...
  commands = [
    ...
    "curl -L -o /var/lib/rancher/k3s/server/manifests/system-upgrade-controller-crd.yaml https://github.com/rancher/system-upgrade-controller/releases/download/${var.system_upgrade_controller_version}/crd.yaml",
  ]
}
